...
return {
	...
	createSession(cred,user,key,session,next){
		if (!user || !key) return next(this.error(401))
		Object.assign(session,cred,{id:user.id,sess:key})
		next()
	},
	storeSession(entId,grp,session,next){
		if (!entId) return next(this.error(401))
