...
pico.def('picGroup', function(){
    ...
    me.showEntity = function(param){
        var name = ('string' === typeof param) ? param : param.name;
        return this.showEntityByIndex(this.getEntityIndex(name, this.hiddenList));
    };

    me.showEntityByIndex = function(i){
        var
        hl = this.hiddenList,
        ent = hl[i];
    ...
    me.showAllEntities = function(){
        for(var i=0, l=this.hiddenList.length; i<l; i++){
            this.showEntityByIndex(0);
        }
    };

    me.hideEntity = function(param){
        var name = ('string' === typeof param) ? param : param.name;
        return this.hideEntityByIndex(this.getEntityIndex(name));
    };

    me.hideEntityByIndex = function(i){
        var
        e = this.entities[i];
        if (!e) return;
    ...
    me.hideAllEntities = function(){
        for(var i=0, l=this.entities.length; i<l; i++){
            this.hideEntityByIndex(0);
        }
    };
