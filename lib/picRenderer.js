...
module.exports = {
    ...
    resizeContext: function(id, x, y, width, height){
        var con = contexts[id];
        if (!con) return;
        con.rect = [x, y, width, height];
    },

    setGlobalAlpha: function(cid, alpha){
        var co = contexts[cid];
        co.alpha = alpha;
        co.isdirty = true;
    },

    setBG: function(cid, bg, clearFunc){
        var co = contexts[cid];
        if (bg){
            co.bg = bg;
    ...
    makeDirty: function(entity, isDirty){
    ...
    captureScreenshot: function(elapsed, evt, entities){
        var
        maxRect = [0, 0, 0, 0],
    ...
    releaseScreenshot: function(elapsed, evt, entities){
        if(offscreenCanvas){
    ...
    drawScreenshot: function(elapsed, evt, entities){
        var
        e = entities[0],
        o = e.getComponent(name);
    ...
    draw: function(elapsed, evt, entities){
        var
        e, o, cid, contextOpt, ctx, rect,
        dComs, dEntities, mod,
        i, l, c, cl;
    ...
    clear: function(elapsed, evt, entities){
        var
