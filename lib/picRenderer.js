pico.def('picRenderer', 'picBase', function(){
    ...
    me.draw = function(elapsed, evt, entities){
        ...
        for(i=0, l=contextOrder.length; i<l; i++){
            ...
            if (contextOpt.resizing) ctx.canvas.width = ctx.canvas.width;

            ctx.save();
            ctx.fillStyle = contextOpt.bg;
            if (contextOpt.resizing) contextOpt.resizing = false;
            else contextOpt.clear.apply(ctx, rect);
            ctx.globalAlpha = contextOpt.alpha;
    ...
    me.clear = function(elapsed, evt, entities){
        ...
        for(i=0, l=contextOrder.length; i<l; i++){
            ...
            if (contextOpt.resizing) cvs.width = cvs.width;
