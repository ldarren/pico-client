...
component format:
{
    picRect: { // content auto generated
        x: 0,
        head: 0
    },
    @picRect: { // '@' auto added
...
pico.def('picTween', 'picBase', function(){
    ...
    me.create = function(ent, com){
        ...
        targetName, target, params, paramName, currentValues,
        j, jl;
        ...
        for (var i=0, l=keys.length; i<l; i++){
            ...
            currentValues = {};
            target = ent.getComponent(target);

            for(j=0, jl=params.length; j<jl; j++){
                paramName = params[j];
    ...
    me.update = function(elapsed, evt, entities){
        ...
        params, paramName, n, nl, pk, p, pl, sign, current, target, rates;
        ...
        for (var i=0, l=entities.length; i<l; i++){
            ...
            for(n=0, nl=keys.length; n<nl; n++){
                ...
                for(p=0,pl=pk.length; p<pl; p++){
                    ...
                    current += sign * rates[paramName] * time;
