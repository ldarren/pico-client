...
pHTTP.prototype = {
	...
	ajax: function(method,route,params,cb){
		...
		var onReceive = function(err, data){
			if (err) {
				self.callback.trigger('network.error', err)
				return cb(err)
			}
			self.callback.trigger('network.recv', null, route, data)
			return cb(null,data)
		}
		...
		this.callback.trigger('network.send', null, route)
	}
}
