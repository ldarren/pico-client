pico.def('piSocials', function(){
    ...
    GOOG = window.GOOG,
    googStateConfig, googCB, googUserId, googState,
...
console.log(JSON.stringify(result));
    ...
    me.googStateLoad = function(key, cb){
        if (!googUserId) {
            return cb(null, window.localStorage.getItem(fbUserId+key));
        }
        googCB = cb;
        GOOG.appState.loadState(key);
    };

    me.googStateUpdate = function(key, state, cb){
        if (!googUserId) {
            return cb(null, window.localStorage.setItem(fbUserId+key, state));
        }
        googState = state;
        googCB = cb;
