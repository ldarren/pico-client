pico.def('piSocials', function(){
    ...
    me.fbReadScore = function(id, cb){
        FB.api(FB_SCORE.replace('USER_ID', id), function(list){
            if (list.length){
                var res = list[0];
                cb(res.user.id, res.score);
            }else{
                cb();
            }
        });
    ...
    me.fbWriteScore = function(id, score, cb){
        FB.api(FB_SCORE.replace('USER_ID', id), 'POST', {score: score}, cb);
    };

    me.fbDeleteScore = function(id, cb){
        FB.api(FB_SCORE.replace('USER_ID', id), 'DELETE', cb);
    };

    me.fbClearAllScores = function(cb){
        FB.api(FB_SCORE.replace('USER_ID', fbAppId), 'DELETE', cb);
    };
});
