...
module.exports = {
    googInit: function(code, extra){
        if (!GOOG) return;
        GOOG.gms.setup(code, extra, googUpdate);
    },

    googLogin: function(cb){
        if (!GOOG) return cb();
        googCB = cb;
        GOOG.gms.signin();
    },

    googStateLoad: function(key, cb){
        if (!googStateConfig) {
    ...
    googStateUpdate: function(key, state){
    ...
    fbProfile: function(id, width, height){
        width = width || 32;
        height = height || width;
        return FB_PROFILE_URL.replace('W', width).replace('H', height).replace('USER_ID', id).replace('TOKEN',fbUserToken);
    },

    fbUserId: function(){return fbUserId}
    fbAccessToken: function(){return fbUserToken}

    fbInit: function(appId, api, cb){
        fbAppId = appId;
        fbApi = api;
        fbCallback = cb;
    ...
    fbReadRequests: function(limit, cb){
        limit = limit || 10000;
    ...
    fbWriteRequests: function(msg, to, data, cb){
        FB.ui({
            method:     'apprequests',
            message:    msg,
    ...
    fbDeleteRequests: function(id, cb){
        FB.api(id, 'delete', cb || dummyCB);
    },

    fbReadScore: function(id, cb){
    ...
    fbWriteAchievement: function(url, cb){
        FB.api('/me/achievements', 'POST', {achievement:url}, cb || dummyCB);
    },

    fbDeleteAchievement: function(){
