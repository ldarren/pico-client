pico.def('piDataNet', function(){
    ...
    acknowledge = function(net, resId){
        ...
        if (net.secretKey){
            var t = net.getServerTime();
            net.acks.push({
                api: 'ack',
                reqId: 0,
                date: t,
                key: CryptoJS.HmacMD5(data, net.secretKey+t).toString(CryptoJS.enc.Base64),
                data: data
            });
        }else{
            net.acks.push({
...
console.log('res: '+JSON.stringify(res));
                ...
                if (net.secretKey){
                    if (res.key !== CryptoJS.HmacMD5(res.data, net.secretKey+res.date).toString(CryptoJS.enc.Base64)){
                        console.error('invalid server key: '+JSON.stringify(res));
                        continue;
                    }
                    if (60000 < Math.abs(res.date - net.getServerTime())){
                        console.error('invalid server time: '+JSON.stringify(res)+' '+net.getServerTime());
                        continue;
                    }
                }
    ...
    PicoNet.prototype.request = function(api, data, cb){
        ...
        if (this.secretKey){
            ...
            this.outbox.push({
                ...
                reqId: ++this.reqId,
                date: t,
                key: CryptoJS.HmacMD5(data, this.secretKey+t).toString(CryptoJS.enc.Base64),
                data: data
            });
        }else{
            this.outbox.push({
