...
formation = function(form, addon, dst, prefix){
    ...
    for (var i=0,elements = form.elements,field; field = elements[i]; i++) {
        ...
        fieldType = 'INPUT' === field.nodeName.toUpperCase() ? field.getAttribute('type').toUpperCase() : 'TEXT'
        if (fieldType === 'FILE') {
...
Net.prototype = {
    ...
    request: function(api, data, addon, cb){
        ...
        case 2:
            if (!(data instanceof Function)) return console.error('wrong request params!')
            cb = data
            data = addon = undefined
            break
        case 3:
            if (!(addon instanceof Function)) return console.error('wrong request params!')
            cb = addon 
            addon = undefined
            break
        case 4: break
