...
console.log('req',req);
            pico.ajax('post', config.pushURL, req, null, function(err, xhr){
                ...
                if (!res || !res.api) return console.error('incomplete response header',res);

                model = pico.modules[res.api.split(MODEL_SEP)[0]];
                model.sync(res);
console.log('res', res);
            });
        }else{
...
pico.def('piDataModel', function(){
    ...
    decodeBufferId = function(key){
        var
        arr = key.split(MODEL_SEP),
        id = {modelId: arr[0]};

        if (arr.length < 2) return id;

        if (-1 === arr[1].indexOf(BUFFER_SEP)){ // key = modelId.index
            id.index = arr[1];
        }else{ // key = modelId.method`reqId
            arr = arr[1].split(BUFFER_SEP);
