...
PicoNet.prototype = {
    request: function(api, data, cb){
        ...
        if (data instanceof HTMLFormElement){
            ...
            var oField, sFieldType, nFile;
            for (var nItem = 0; nItem < elements.length; nItem++) {
                oField = elements[nItem];
                if (!oField.hasAttribute("name")) continue;
                sFieldType = oField.nodeName.toUpperCase() === "INPUT" ? oField.getAttribute("type").toUpperCase() : "TEXT";
                if (sFieldType === "FILE") {
                    for (nFile = 0; nFile < oField.files.length; fd.append('data/'+oField.name, oField.files[nFile++]));
                } else if ((sFieldType !== "RADIO" && sFieldType !== "CHECKBOX") || oField.checked) {
                    fb.append('data/'+oField.name, oField.value);
                }
            }
            ...
            if (this.secretKey){
                ...
                fd.append('date', t);
                fd.append('key', CryptoJS.HmacMD5(json, this.secretKey+t).toString(CryptoJS.enc.Base64));
            }
