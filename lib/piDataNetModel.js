...
onResponse = function(err, xhr, net){
    ...
    if (2 === xhr.readyState){
        net.channelId = xhr.getResponseHeader(CHANNEL);
...
console.debug('res: '+JSON.stringify(res));
    ...
    if (isOnline && (net.uploads.length || net.outbox.length || net.acks.length)){
        ...
        if (net.channelId){
            headers = {
                'X-Channel': net.channelId
            }
console.debug('header: '+JSON.stringify(headers));
        }
...
Net.prototype = {
    formation: function(form, dst, prefix){
        ...
        var append = dst instanceof FormData ? appendFD : appendObj;
        prefix = prefix || '';
