...
console.debug('res: '+JSON.stringify(res));
    ...
    if (isOnline && (net.uploads.length || net.outbox.length || net.acks.length)){
        ...
        if (net.uploads.length){
            pico.ajax('post', net.url, net.uploads.shift(), headers, onResponse, net);
...
PicoNet.prototype = {
    submit: function(form, cb){
        ...
        for (var i=0, l=elements.length; i < l; i++) {
            ...
            if (fieldType === 'FILE') {
                for (f = 0, fl=field.files.length; f < fl; fd.append('data/'+field.name, field.files[f++]));
            } else if ((fieldType !== 'RADIO' && fieldType !== 'CHECKBOX') || field.checked) {
                fd.append('data/'+field.name, field.value);
            }
        }

        fd.append('api', form.action.substr(form.baseURI.length));
