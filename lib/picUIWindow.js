...
pico.def('picUIContent', 'picBase', function(){
    ...
    copyMeshUI = function(ui, template, width, height){
        ...
        else if (val < 8) ui[MESH_W] = width * val;
        val = ui[MESH_H];
    ...
    me.fillIconText = function(ctx, ts, text, x, y, width, height, options){
        var
        align = 1,
        baseline = 1,
        padX = 0, padY = 0,
        ...
        for(i=0,j=lineWords.length; i<j; i++){
            ...
            switch(align){
                ...
                case 1: offX = x + padX + Floor((width- lineW)/2); break;
                case 2: offX = x + width - padX - lineW; break;
            }
            switch(baseline){
                case 0: offY = y + padY + (lineH * i); break;
                case 1: offY = y + padY + (lineH * i) + lineH/2; break;
                case 2: offY = y + padY + (lineH * i) + lineH; break;
            }
            ...
            for(k=0,l=words.length; k<l; k++){
                ...
                if ('string' === typeof word){
                    ...
                    dim = ts.getDimension(word);
                    ts.draw(ctx, word, offX, offY - (dim[3]/2));
                }
                offX += lens[k];
            }
