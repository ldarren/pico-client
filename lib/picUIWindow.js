...
pico.def('picUIContent', 'picBase', function(){
    ...
    copyMeshUI = function(ui, template, width, height){
        ...
        if (!ui[MESH_CELL_PT]) ui[MESH_CELL_PT] = me.TOP;
        if (!ui[MESH_UI_PT]) ui[MESH_UI_PT] = me.TOP;
    ...
    me.fillWrapText = function(ctx, text, x, y, maxWidth, lineHeight, options){
        var
        align = 0,
        padX = 0,
        realX,
        ...
        for(var s=0,i=0,j=0; -1 !== j; i=j+1) {
            ...
            metrics = ctx.measureText(j > 0 ? text.substring(s, j+1) : text.substring(s));

            switch(align){
                case 0: realX = x + padX; break;
                case 1: realX = x + padX + Floor((maxWidth - metrics.width)/2); break;
                case 2: realX = x + maxWidth - padX; break;
            }
console.log(align, realX, maxWidth, metrics.width, padX, x);

            if (-1 === j){
                ctx.fillText(text.substring(s), realX, y);
                break;
            }

            if (metrics.width > maxWidth) {
                ctx.fillText(text.substring(s, i), realX, y);
                s = i;
                y += lineHeight;
            }
        }
