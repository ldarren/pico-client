pico.def('picUIWindow', 'picBase', function(){
    ...
    drawBorderCell = function(ctx, rect, cell){
        ...
        for(i=1,l=cell.length; i<l; r++){
            ...
            switch(ui.type){
                case ui.TEXT:
                    drawBorderText(ctx, [rect[0]+rect[3]*(i-1), rect[1], w, rect[3]], row[i]);
                    break;
                case ui.TILE:
                    drawBorderTile(ctx, [rect[0]+rect[3]*(i-1), rect[1], w, rect[3]], row[i]);
                    break;
                case ui.BUTTON:
                    drawBorderButton(ctx, [rect[0]+rect[3]*(i-1), rect[1], w, rect[3]], row[i]);
                    break;
                case ui.LAYOUT:
                    drawBorderLayout(ctx, [rect[0]+rect[3]*(i-1), rect[1], w, rect[3]], row[i]);
                    break;
            }
        }
        ctx.restore();
    },
    drawBorderText = function(ctx, rect, text){
    },
