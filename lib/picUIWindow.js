...
pico.def('picUIContent', 'picBase', function(){
    ...
    MESH_BTN='button',MESH_DRAG='drag',MESH_DROP='drop',DEFAULT='default',
...
//ctx.stroke();
    ...
    doDropping = function(focus, targetCell, uiRect, ui, x, y){
        ...
        if (sourceUI && targetCell[0][MESH_DROP]){
            sourceCell = sourceUI.cell;
            sourceIdx = sourceCell.indexOf(sourceUI);
            sourceCell.splice(sourceIdx, 1);
            if (ui && ui[MESH_DRAG]){
                targetIdx = targetCell.indexOf(ui)
                targetCell.splice(targetIdx, 1, sourceUI);
                sourceCell.splice(sourceIdx, 0, ui);
            }else{
            ...
            case WALK_UI:
                ...
                case TYPE_CUSTOM:
                    ...
                    if (x > uiRect[0] && x < uiRect[0]+uiRect[2] && y > uiRect[1] && y < uiRect[1]+uiRect[3]){
                        if (state){
                            ret = doButtonDown(focus, cell, uiRect, ui, x, y);
                        }else{
                            ret = doButtonUp(focus, cell, uiRect, ui, x, y);
                            if (ret) cb.call(this, me.CUSTOM_CLICK, ui);
                        }
                    }
                    break;
                }
        ...
        return walkMeshUI.call(this, rect, meshUI, function(type){
            ...
            case WALK_UI:
                ...
                case TYPE_CUSTOM:
                    uiRect = cb.call(this, me.CUSTOM_BOUND, cellRect, ui, scale);
                    if (x > uiRect[0] && x < uiRect[0]+uiRect[2] && y > uiRect[1] && y < uiRect[1]+uiRect[3]){
                        ret = doPicking(focus, cell, uiRect, ui, x, y);
console.log('pickMeshUI: '+ret);
                    }
                    break;
                }
                break;
...
console.log('dragMeshUI x:'+focus[FOCUS_DRAG_X]+' y:'+focus[FOCUS_DRAG_Y]+' x:'+x+' y:'+y);
            return true;
        }
        return false;
    },
        ...
        return walkMeshUI.call(this, rect, meshUI, function(type){
            ...
            case WALK_UI:
                ...
                case TYPE_CUSTOM:
                    uiRect = cb.call(this, me.CUSTOM_BOUND, cellRect, ui, scale);
                    if (x > uiRect[0] && x < uiRect[0]+uiRect[2] && y > uiRect[1] && y < uiRect[1]+uiRect[3]){
                        ret = doDropping(focus, cell, uiRect, ui, x, y);
console.log('dropMeshUI');
                    }
                    break;
                }
                break;
    ...
    me.pickMeshUI = function(x, y, com, comBox, scale, cb){
        var ret = pickMeshUI.call(this, x, y, [0, 0, comBox.width, comBox.height], com.layout, com.focus, scale, cb);
        console.log('me.pickMeshUI: '+ret);
        return ret;
    };
