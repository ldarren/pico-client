...
pico.def('picUIContent', 'picBase', function(){
    ...
    drawMeshRow = function(ctx, rect, cells){
        ...
        for(i=1,l=cells.length; i<l; i++){
            drawMeshCell(ctx, [rect[0]+w*(i-1), rect[1], w, rect[3]], cells[i]);
        }
        ctx.restore();
    },
    drawMeshCell = function(ctx, rect, uis){
        ...
        for(i=1,l=uis.length; i<l; i++){
            ...
            switch(ui.type){
                case me.TYPE_TEXT:
                    drawMeshText(ctx, rect, ui);
                    break;
                case me.TYPE_TILE:
                    drawMeshTile(ctx, rect, ui);
                    break;
                case me.TYPE_BUTTON:
                    drawMeshButton(ctx, rect, ui);
                    break;
                case me.TYPE_CUSTOM:
                    ui.cb(ctx, rect, ui);
                    break;
                case me.TYPE_MESH:
                    drawMeshUI(ctx, rect, ui);
                    break;
            }
        }
        ctx.restore();
    ...
    drawMeshUI = function(ctx, rect, meshUI){
        ...
        for(i=1,l=rows.length; i<l; i++){
            drawMeshRow(ctx, [inner[0], inner[1]+(h*(i-1)), inner[2], h], rows[i]);
        }
        ctx.restore();
    },
                    ...
                    switch(ui.type){
                        case me.TYPE_BUTTON:
                            ...
                            if (x > region[0] && x < region[0]+region[2] && y > region[1] && y < region[1]+region[3]){
                                ui.state = state;
                                if (!state) ui.cb(ui.evt);
                                return true;
                            }
                            break;
                        case me.TYPE_MESH:
                            if (clickMeshUI(x, y, state, region, ui)) return true;
                            break;
                    }
                }
            }
    ...
    me.createMeshButton = function(cell, cellPt, uiPt, pad, width, height, upColor, downColor, label, cb, evt){
        var ui = {
            type: me.TYPE_BUTTON,
            cellPt: cellPt,
            uiPt: uiPt,
    ...
    me.createMeshCustom = function(cell, cellPt, uiPt, pad, width, height, cb, userData){
        var ui = {
            type: me.TYPE_CUSTOM,
            cellPt: cellPt,
            uiPt: uiPt,
    ...
    me.loadMeshTemplates = function(path, cb){
