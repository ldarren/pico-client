...
pico.def('picUIContent', 'picBase', function(){
    ...
    drawMeshText = function(ctx, rect, meshText, ts, tileScale){
        ...
        me.fillIconText(ctx, ts, meshText.text, rect[0], rect[1], rect[2], rect[3], {align:align,baseline:baseline});
    },
        ...
        for(r=1,rl=rows.length; r<rl; r++){
            ...
            for(c=1,cl=cells.length; c<cl; c++){
                ...
                for(u=1,ul=uis.length; u<ul; u++){
                    ui = uis[u];
                    switch(ui.type){
                    case me.TYPE_TEXT: drawMeshText.call(this, ctx, rect, ui, tileSets[ui.tileSet], tileScale); break;
                    case me.TYPE_TILE: drawMeshTile.call(this, ctx, rect, ui, tileSets[ui.tileSet], tileScale); break;
                    case me.TYPE_BUTTON: drawMeshButton.call(this, ctx, rect, ui, tileSets[ui.tileSet], tileScale); break;
                    case me.TYPE_CUSTOM: this.signal('draw', [ctx, setInner(rect, ui), ui, tileSets[ui.tileSet], tileScale]); break;
                    case me.TYPE_MESH: drawMeshUI.call(this, ctx, rect, ui, tileSets, tileScale); break;
                    }
                }
                ctx.restore();
            }
    ...
    me.createMeshUI = function(cell, cellPt, uiPt, pad, width, height, option){
        ...
        if (cell){
            cell.push(ui);
            // root 
            if (ui[MESH_W] < 8) ui[MESH_W] = ui[MESH_W] * width;
            if (ui[MESH_H] < 8) ui[MESH_H] = ui[MESH_H] * height;
        }
        return ui;
    };
