...
ctx.fillRect(rect[0], rect[1], rect[2], rect[3]);
                for(u=1,ul=uis.length; u<ul; u++){
                    ui = uis[u];
var iii = setInner(rect, ui);
ctx.beginPath();
                    switch(ui.type){
                    case me.TYPE_TEXT:
ctx.fillStyle = 'red';
ctx.strokeStyle = 'red';
                        drawMeshText.call(this, ctx, rect, ui, tileSets[ui.tileSet || DEFAULT], tileScale);
                        break;
                    case me.TYPE_TILE:
ctx.fillStyle = 'white';
ctx.strokeStyle = 'white';
                        drawMeshTile.call(this, ctx, rect, ui, tileSets[ui.tileSet || DEFAULT], tileScale);
                        break;
                    case me.TYPE_CUSTOM:
ctx.fillStyle = 'blue';
ctx.strokeStyle = 'blue';
                        this.signal('draw', [ctx, setInner(rect, ui, tileScale), ui, tileSets[ui.tileSet || DEFAULT], tileScale]);
                        break;
                    case me.TYPE_MESH:
                        drawMeshUI.call(this, ctx, rect, ui, tileSets, tileScale, font);
                        break;
                    }
ctx.arc(iii[0], iii[1], 2, 0, 2 * Math.PI, false);
ctx.fill();
ctx.rect(iii[0], iii[1], iii[2], iii[3]);
ctx.stroke();
                }
                ctx.restore();
            }
                ...
                for(u=1,ul=cell.length; u<ul; u++){
                    ...
                    if (ui[MESH_BUTTON]){
                        ...
                        if (x > region[0] && x < region[0]+region[2] && y > region[1] && y < region[1]+region[3]){
                            ui[MESH_BUTTON] = state;
                            if (!state) this.signal('click', [ui]);
                            return true;
                        }
                    }
                    if (ui.type == me.TYPE_MESH){
    ...
    createMeshUI = function(template){
    ...
    copyMeshUI = function(ui, template, width, height){
        ...
        var w = ui[MESH_W] < 8 ? ui[MESH_W] * width : ui[MESH_W];
        var h = ui[MESH_H] < 8 ? ui[MESH_H] * height : ui[MESH_H];
