...
pico.def('picUIContent', 'picBase', function(){
    ...
    drawMeshText = function(ctx, rect, meshText, ts, tileScale, fontScale){
    ...
    drawMeshTile = function(ctx, rect, meshTile, tileSet, tileScale, fontScale){
        var inner = setInner(rect, meshTile);

        tileSet.draw(ctx, meshTile.tileId, inner[0], inner[1], inner[2], inner[3]);
    },
    drawMeshButton = function(ctx, rect, meshButton, tileSet, tileScale, fontScale){
    ...
    drawMeshUI = function(ctx, rect, meshUI, tileSets, tileScale, fontScale){
        var
        ...
        for(r=1,rl=rows.length; r<rl; r++){
            ...
            for(c=1,cl=cells.length; c<cl; c++){
                ...
                for(u=1,ul=uis.length; u<ul; u++){
                    ...
                    switch(ui.type){
                        case me.TYPE_TEXT:
                            drawMeshText.call(this, ctx, rect, ui, tileSets[ui.tileSet], tileScale, fontScale);
                            break;
                        case me.TYPE_TILE:
                            drawMeshTile.call(this, ctx, rect, ui, tileSets[ui.tileSet], tileScale, fontScale);
                            break;
                        case me.TYPE_BUTTON:
                            drawMeshButton.call(this, ctx, rect, ui, tileSets[ui.tileSet], tileScale, fontScale);
                            break;
                        case me.TYPE_CUSTOM:
                            this.signal('draw', [ctx, setInner(rect, ui), ui, tileSets[ui.tileSet], tileScale, fontScale]);
                            break;
                        case me.TYPE_MESH:
                            drawMeshUI.call(this, ctx, rect, ui, tileSets, tileScale, fontScale);
                            break;
                    }
                }
                ctx.restore();
    ...
    copyMeshUI = function(ui, template, width, height){
        ...
        else if (val < 2) ui[MESH_W] = width * val;
        val = ui[MESH_H];
        if (!val) ui[MESH_H] = height;
        else if (val < 2) ui[MESH_H] = height * val;
    ...
    me.drawMeshUI = function(ctx, rect, meshUI, tileSets, tileScale, fontScale){
        //always start from 0,0 not rect.x, rect.y. it will be offset in uiWindow
        drawMeshUI.call(this, ctx, [0, 0, rect.width, rect.height], meshUI, tileSets, tileScale, fontScale);
    };
    ...
    me.fillIconText = function(ctx, ts, text, x, y, maxWidth, lineHeight, options){
        var
        align = 0,
        padX = 0,
        symbol = '`',
        ...
        for(var i=0,j=0; -1 !== j;) {
            ...
            if (symbol === text.charAt(i)){
                str = j > 0 ? text.substring(i, j) : text.substring(i);
                i = j;
                str = parseInt(str.substring(1));
                dim = ts.getDimension(str);
                len = dim[2];
            }else{
                str = j > 0 ? text.substring(i, j + 1) : text.substring(i);
                i = j + 1;
                metrics = ctx.measureText(str);
                len = metrics.width;
            }
            if (len > maxWidth) break; // maxWidth too small
            if (lineW + len > maxWidth || j < 0){
                if (j < 0){
                    words.push(str);
                    lens.push(len);
                    lineW += len;
                ...
                switch(align){
                    case 0: offX = x + padX; break;
                    case 1: offX = x + padX + Floor((maxWidth - lineW)/2); break;
                    case 2: offX = x + maxWidth - padX - lineW; break;
                }

                for(k=0,l=words.length; k<l; k++){
                    word=words[k];
                    if ('string' === typeof word){
                        ctx.fillText(word, offX, y);
                    }else{
                        ts.draw(ctx, word, offX, y);
                    }
                    offX += lens[k];
                }
                lineW = 0;
