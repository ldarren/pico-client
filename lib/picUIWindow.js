...
pico.def('picUIContent', 'picBase', function(){
    ...
    drawMeshText = function(ctx, rect, meshText, ts, tileScale, fontScale){
        ...
        me.fillIconText(ctx, ts, meshText.text, rect[0], rect[1], meshText.w, meshText.h, {align:align,baseline:baseline});
    },
        ...
        for(r=1,rl=rows.length; r<rl; r++){
            ...
            for(c=1,cl=cells.length; c<cl; c++){
                ...
                for(u=1,ul=uis.length; u<ul; u++){
                    ui = uis[u];
                    switch(ui.type){
                    case me.TYPE_TEXT: drawMeshText.call(this, ctx, rect, ui, tileSets[ui.tileSet], tileScale, fontScale); break;
                    case me.TYPE_TILE: drawMeshTile.call(this, ctx, rect, ui, tileSets[ui.tileSet], tileScale, fontScale); break;
                    case me.TYPE_BUTTON: drawMeshButton.call(this, ctx, rect, ui, tileSets[ui.tileSet], tileScale, fontScale); break;
                    case me.TYPE_CUSTOM: this.signal('draw', [ctx, setInner(rect, ui), ui, tileSets[ui.tileSet], tileScale, fontScale]); break;
                    case me.TYPE_MESH: drawMeshUI.call(this, ctx, rect, ui, tileSets, tileScale, fontScale); break;
                    }
                }
                ctx.restore();
            }
        ...
        for(i=0,j=lineWords.length; i<j; i++){
            ...
            for(k=0,l=words.length; k<l; k++){
                ...
                if ('string' === typeof word){
                    ...
                    dim = ts.getDimension(word);
                    ts.draw(ctx, word, offX, offY - (dim[3]*iconOff));
                }
                offX += lens[k];
            }
