    ...
    me.handleEvent = function(evt){
        ...
        switch(evt.type){
            ...
            case 'mouseup':
                ...
                if (fingerTimerId){
                    fingerBucket.push([me.FINGER_UP, [evt, evt.clientX, evt.clientY]]);
                }else{
            ...
            case 'touchcancel':
                ...
                var now = Date.now();
                if (DDC_TIMEOUT > now - fingerTimer){
                    window.clearTimeout(fingerTimerId);
                    fingerTimer = fingerTimerId = 0;
                    me.signal(me.FINGER_TWICE, [evt, evt.clientX, evt.clientY]);
                }
                fingerTimer = now;
                fingerTimerId = window.setTimeout(function(){
