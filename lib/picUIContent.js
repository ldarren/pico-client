...
module.exports = {
    calcUIRect:calcUIRect;

    createMeshOption: function(strokeStyle, fillStyle, font, alpha, lineWidth, lineCap, lineJoin){
        return {
            font: font,
    ...
    createMeshRow: function(rows, option){
        var newRow = [option || {}];
        rows.push(newRow);
        return newRow;
    };

    createMeshCell: function(row, option){
        var cell = [option || {}];
        row.push(cell);
        return cell;
    };

    createMeshText: function(cell, cellPt, uiPt, pad, width, height, text){
        var ui = {
            type: TYPE_TEXT,
            cellPt: cellPt,
            uiPt: uiPt,
    ...
    createMeshTile: function(cell, cellPt, uiPt, pad, width, height, tileSet, tileId){
        var ui = {
            type: TYPE_TILE,
            cellPt: cellPt,
            uiPt: uiPt,
    ...
    createMeshCustom: function(cell, cellPt, uiPt, pad, width, height, button, drag, userData){
        var ui = {
            type: TYPE_CUSTOM,
            cellPt: cellPt,
            uiPt: uiPt,
    ...
    createMeshUI: function(cell, cellPt, uiPt, pad, width, height, option){
        var ui = {
            type: TYPE_MESH,
            cellPt: cellPt,
            uiPt: uiPt,
    ...
    loadMeshUITemplates: function(path, cb){
    ...
    createMeshUIFromTemplate: function(name, width, height){
        var 
    ...
    drawMeshUI: function(ctx, tileSets, ent, com, comBox, scale, cb){
...
//ctx.stroke();
     ...
     */
    clickMeshUI: function(x, y, state, ent, com, comBox, scale, cb){
    ...
    pickMeshUI: function(x, y, ent, com, comBox, scale, cb){
        var
        rect = [0, 0, comBox.width, comBox.height],
        meshUI = com.layout,
        focus = com.focus,
    ...
    dragMeshUI: function(x, y, ent, com, comBox, scale, cb){
        var
        focus = com.focus,
    ...
    dropMeshUI: function(x, y, ent, com, comBox, scale, cb){
        var
        rect = [0, 0, comBox.width, comBox.height],
        meshUI = com.layout,
        focus = com.focus,
    ...
    generateGridLayout: function(rect, itemWidth, itemHeight, rowCount, colCount){
        colCount = colCount || 1;
        rowCount = rowCount || 1;
    ...
    fillWrapText: function(ctx, text, x, y, maxWidth, lineHeight){
        var metrics;
    ...
    fillIconText: function(ctx, tss, text, rect, scale, options){
        var
        align = 2,
        baseline = 2,
        padX = 0, padY = 0,
    ...
    drawButton: function(ctx, tss, label, rect, scale, fontColor, faceColor, borderColor, baseline){
    ...
    pick: function(ent, x, y){
        // place holder
        return false;
    };

    drag: function(ent, x, y){
        // place holder
        return false;
    };

    drop: function(ent, x, y){
        // place holder
        return false;
    };

    click: function(ent, evt, state){
        // place holder
        return false;
    };

    draw: function(ctx, ent, clip){
        // place holder
    };

    drawScreenshot: function(ctx, ent, clip, bitmap){
        // place holder
    }
};
