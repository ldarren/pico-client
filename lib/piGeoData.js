pico.def('piGeoData', function(){
    ...
    function boundary(lat, lng, accuracy){
        var
        dLat = 180 / Math.pow(2, accuracy),
        dLng = 360 / Math.pow(2, accuracy),
        bitfield=[],
        data = [ [lat, -90, 90], [lng, -180, 180] ],
        tlLat, tlLng;
    ...
    this.getBoundary = function(lat, lng){
    ...
    this.startWatch = function(range, cb){
        if (navigator.geolocation){
